...
**Open in VS Code or [GitHub Codespaces](https://github.com/features/codespaces):** If you use Codespaces (or Dev Containers), the [.devcontainer](.devcontainer/devcontainer.json) will install Node, Go, Python, Java, .NET, Rust, and Ruby and run `./scripts/install.sh all` so the repo is ready to go. Otherwise, open the folder for your language and install dependencies (see below or that folder’s README).
...
- **Tasks:** **Terminal → Run Task…** (or `Cmd+Shift+B` / `Ctrl+Shift+B`), then choose **Tests: TypeScript**, **Tests: Go**, **Tests: Python**, **Tests: Java**, **Tests: C#**, **Tests: Rust**, **Tests: Ruby**, or **Tests: All** to run every language’s tests.
...
- **Recommended extensions:** When opening in VS Code or Codespaces, accept the recommended extensions (TypeScript/ESLint, Go, Python, Java, C#, Vitest, Rust, Ruby) for the best experience.
...
Languages: `typescript`, `go`, `python`, `java`, `csharp`, `rust`, `ruby`.
