...
3. Optional: run `bundle install` in the `ruby` directory (Minitest is in the standard library; Gemfile is minimal).
...
Or without Bundler (if you have no Gemfile deps):

```bash
ruby -Ilib:test test/rate_limiter_test.rb
```
...
Tests set a user's tier via `USER_TIERS` (e.g. `USER_TIERS["user1"] = "free"`). The rate limiter reads from this map to decide which limits apply. Each test clears `USER_TIERS` and `REQUEST_TIMESTAMPS` in `setup`.
